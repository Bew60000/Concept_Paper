...
const State02_Assign_work = ({ isOpen, closeModal, selectedForm, studentData, teacherData, UpdateStatus }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center">
      <div className="bg-white p-12 rounded-lg w-11/12 rounded-full min-h-[650px] overflow-y-auto mt-16">
        {selectedForm && (
          ...
          {dataUser.map((user, index) => (
            <div key={index} className='mx-12'>
              <div className="bg-gray-100 p-3 rounded-xl w-full mb-4">
                <div className="grid grid-cols-12 gap-4 items-center">
                  <div className="col-span-3 text-start">
                    <p className="text-gray-800 text-center font-bold m-0">{ index + 1}.{user.name}&nbsp;{user.lastname}</p>                    
                  </div>
